...
class ClientCharacter {
	...
	private function e_requestPvpBattle(data:Dynamic):Void {
		...
		client.socket.volatile.emit('requestPvpBattleConfirm', { } );
		requestedPlayer.socket.volatile.emit('requestPvpBattle', {requestUsername: client.username});
	}
	
	private function e_requestPvpBattleAccept(data:Dynamic):Void {
		if (battle != null) return;
		
		var requestedFirstPlayer = GameServer.getCharByUsername(Std.string(requestedPvpUsername));
