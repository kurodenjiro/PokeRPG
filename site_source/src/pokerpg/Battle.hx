...
class Battle {
	...
	public var enemyTrainerImg:ImageResource;
	public var background:ImageResource;
	
	public var backgroundImage:ImageResource;
	public var enemyBaseImage:ImageResource;
	public var playerBaseImage:ImageResource;
	...
	private var animStep2:Int;
	
	public var canRenderEnemy:Bool;
	public var canRenderEnemyTrainer:Bool;
	...
	public function new(data:BattleInitData) {
		...
		switch(type) {
			...
			case BATTLE_VERSUS:
				...
				enemyTrainerImg = Game.curGame.getImage('resources/chars/frontsprite/'+data.info.players[myId == 0 ? 1:0].type+'.png');
				
			default: throw "Invalid battle type";
		}
		
		//background = Game.curGame.getImage('resources/ui/battle_background1.png');
		background = Game.curGame.getImage('resources/ui/min/battle_background1.png');
		backgroundImage = Game.curGame.getImage('resources/battle/battlebacks/battlebg0.png');
		enemyBaseImage = Game.curGame.getImage('resources/battle/battlebacks/enemybase0.png');
		playerBaseImage = Game.curGame.getImage('resources/battle/battlebacks/playerbase0.png');
		...
		if (type == BATTLE_VERSUS) {
			canRenderEnemy = false;
			canRenderEnemyTrainer = true;
		}
		else if (type == BATTLE_WILD) {
			canRenderEnemy = true;
			canRenderEnemyTrainer = false;
		}
	...
	public function render(ctx:CanvasRenderingContext2D):Void {
		...
		if (step == BATTLE_STEP_LEARN_MOVE) {
			...
			if (backgroundImage.obj.width != 0) {
				// Background Pattern
				//ctx.drawImage(backgroundImage.obj, 0, 0);
				var backgroundPattern = ctx.createPattern(backgroundImage.obj, "repeat-x");
				ctx.fillStyle = backgroundPattern;
				ctx.fillRect(0, 0, 480, 224);
				
				ctx.drawImage(enemyBaseImage.obj, 224, 85);
				ctx.drawImage(playerBaseImage.obj, 0, 192);
			}
			ctx.lineWidth = 2;
			...
			if (step == BATTLE_STEP_TRANSITION || step == BATTLE_STEP_POKEMON_APPEARED_TMP || step == BATTLE_STEP_POKEMON_APPEARED) {
				...
				if (canRenderEnemyTrainer) {
					ctx.drawImage(enemyTrainerImg.obj, 360, 0);
				}
			}else if (step == BATTLE_STEP_GO_POKEMON_PVP) {
				if (Game.getRes('playerBacksprite').obj != null) {
					ctx.drawImage(Game.getRes('playerBacksprite').obj, 0, 0, 128, 128, 60, 96, 128, 128);
				}
				drawGoEnemyPokemonAnimation(ctx);
			...
			&& step != BATTLE_STEP_GO_POKEMON_PVP) {
			...
			if (step == BATTLE_STEP_POKEMON_APPEARED_TMP) {
				step = BATTLE_STEP_POKEMON_APPEARED;
				if (type == BATTLE_VERSUS) {
					setBattleText('You are challanged by ' + enemyUsername + '!', -1, function():Void{
						var pk = curPokemon;
						
						var tmpName:String = Util.getPokemonDisplayName(enemyPokemon);
						setBattleText(enemyUsername + ' send out ' + tmpName.substr('FOE '.length) + '!');
						step = BATTLE_STEP_GO_POKEMON_PVP;
						animStep2 = 0;
						selectedMove = 0;
					});
	...
	private function drawGoEnemyPokemonAnimation(ctx:CanvasRenderingContext2D):Void {
		if (animStep2 >= 4 && animStep2 < 25) {
			var ballAnimation = [
				[400, 79, 1, 0],
				[398, 75, 1, 0],
			...
			ctx.save();
			//ctx.translate(90, 0);
			ctx.globalAlpha = ballAnimation[animStep2-4][4];
			ctx.translate(ballAnimation[animStep2-4][0], ballAnimation[animStep2-4][1]);
			ctx.scale(ballAnimation[animStep2-4][2], ballAnimation[animStep2-4][2]);
			ctx.rotate(animStep2 / 17 * Math.PI * 2);
			
			ctx.drawImage(Game.getRes('battlePokeballs').obj, 64, 32 * ballAnimation[animStep2-4][3], 32, 32, -16, -16, 32, 32);
			ctx.restore();
		}
		
		if(Math.floor(animStep2 / 4) < 5){
			ctx.save();
			ctx.beginPath();
			ctx.moveTo(360, 0);
			ctx.lineTo(498, 0);
			...
			ctx.globalAlpha = Util.clamp(1 - (animStep2 / 20), 0, 1);
			
			ctx.drawImage(enemyTrainerImg.obj, 0, 0, 128, 128, 360 + animStep2 * 5, 0, 128, 128);
			ctx.restore();
		}
		
		if(animStep2 > 21 && animStep2 < 35){
			var perc = Math.min((animStep2 - 21) / 10, 1);
			...
			tmpCtx.translate(423, 125);
			tmpCtx.scale(perc, perc);
			tmpCtx.drawImage(enemyPokemon.sprite.obj, -64, -96);
			tmpCtx.restore();
			
			
			ctx.save();
			ctx.translate(423, 125);
			//ctx.translate(124, 192);
			ctx.scale(perc, perc);
			ctx.drawImage(enemyPokemon.sprite.obj, -64, -96);
	...
	private function drawPokemonStatus(ctx:CanvasRenderingContext2D) {
		...
		var HpX = 382 - (curPokemon.hp >= 100 ? 8 : 0);
		...
		Renderer.drawShadowText2(ctx, Std.string(curPokemon.hp), HpX, 208, 'rgb(64,64,64)', 'rgb(216,208,176)');
	...
	public function runQueue(force:Bool = false):Void {
		...
		case "win":
			if (type == Battle.BATTLE_TRAINER) {
				setBattleText((getPlayerTeam(action.player) == myTeam ? Game.username : enemyUsername) + ' has defeated ' + (getPlayerTeam(action.player) != myTeam ? Game.username : enemyUsername), -1, function():Void {
					finish();
				});
			}
			else {
