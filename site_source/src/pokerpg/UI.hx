...
class UI {
	...
	static public var mouseOverUsernamesFix:Array<String>;
	static public var renderRightMouseBox:Bool;
	...
	static public function setupHUD():Void {
		
	}
	
	static public function getMaxUsernameWidth():Float {
		if (UI.mouseOverUsernamesFix.length < 1) return 100;
		var x:Float = 0;
		for (i in 0...UI.mouseOverUsernamesFix.length) {
			if (Main.ctx.measureText(UI.mouseOverUsernamesFix[i]).width > x) { x = Main.ctx.measureText(UI.mouseOverUsernamesFix[i]).width; }
		}
		return x;
	}
