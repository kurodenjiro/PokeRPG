...
class Chat {
	...
	static public function sendMessage():Void {
		...
		if (str.indexOf('/addpokemon ') == 0 && Game.accountLevel >= 70) {
			Connection.socket.emit('adminAddPokemon', { id:str.substr('/addpokemon '.length) } );
			resetChat();
			Main.jq(Main.onScreenCanvas).focus();
			return;
		}
		
		if (str.indexOf('/unstuck') == 0 && !Game.curGame.inBattle) {
			Renderer.startTransition(new FadeOut(10)).onComplete = function():Void {
				Connection.socket.emit('unstuck', { username:Game.username } );
				resetChat();
				Main.jq(Main.onScreenCanvas).focus();
		...
		if (str.indexOf('/pvp ') == 0 && !Game.curGame.inBattle) {
			Connection.socket.emit('requestPvpBattle', { username:str.substr('/pvp '.length) } );
			resetChat();
			Main.jq(Main.onScreenCanvas).focus();
			return;
		}
		
		if (str.indexOf('/accpvp') == 0 && !Game.curGame.inBattle) {
			Connection.socket.emit('requestPvpBattleAccept', { } );
			resetChat();
			Main.jq(Main.onScreenCanvas).focus();
			return;
		}
