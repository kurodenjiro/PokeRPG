...
class Battle {
	...
	public function init():Void {
		for (p in players) {
			...
			/*Main.log('Player Emit ' + p.client.username);
			Main.log('Player Id ' + p.id);
			Main.log('Player Team ' + p.team);*/
			p.client.socket.emit('battleInit', {
				...
				info: {
					players: Lambda.array(Lambda.map(players, function(otherPlayer:BattlePlayer) {
						return {
							username: (otherPlayer.client != null ? otherPlayer.client.username : ''),
							pokemon: otherPlayer.pokemon.generateNetworkObject(otherPlayer == p),
							type: (otherPlayer.client != null ? otherPlayer.client.character.type : '')
						}
					}))
				}
			});
	...
	private function checkFainted() {
		for (p in players) {
			...
			if (battleType == BATTLE_VERSUS) exp = 0;
	...
	public function processPlayerTurn(p:BattlePlayer):Void {
		switch(p.action) {
			case TRun:
				...
				if (battleType == BATTLE_VERSUS) success = false;
