...
class CPokemon extends GameObject {
	...
	inline public static var POKEMON_WIDTH_BIG = 128;
	inline public static var POKEMON_HEIGHT_BIG = 128;
	
	public var image:ImageResource;
	
	public var shiny:Bool;
	public var idPokemon:String;
	...
	public function new(id:String, x:Int, y:Int, dir = Game.DIR_DOWN, shiny = false) {
		...
		if (id.length == 1) strId = '00';
		else if (id.length == 2) strId = '0';
		
		image = id == null ? null : Game.curGame.getImage('resources/followers/' + strId + id + (shiny == true ? 's' : '') + '_0.png');
	...
	override public function render(ctx:CanvasRenderingContext2D):Void {
		...
		if (image != null && image.loaded) {
			...
			if (idPokemon == '644') {
				...
				var renderX:Int, renderY:Int;
				
				renderX = renderPosX + offsetX;
				renderY = renderPosY + offsetY + (canIdleJump && (Renderer.numRTicks + randInt) % 10 < 5 ? -2 : 0);
				if (followDirection == 2) { // RIGHT
					renderX -= 80;
					renderY -= 48;
 				}
				else if (followDirection == 1) { // LEFT
					renderX += 16;
					renderY -= 48;
				}
				else if (followDirection == 0) { // DOWN
					renderX -= 32;
					renderY -= 32;
				}
				else if (followDirection == 3) { // UP
					renderX -= 32;
					renderY -= 16;
				}
				ctx.drawImage(image.obj, Math.floor(((Renderer.numRTicks + randInt) % 20)/5) * POKEMON_WIDTH_BIG, POKEMON_HEIGHT_BIG * followDirection, POKEMON_WIDTH_BIG, POKEMON_HEIGHT_BIG, renderX, renderY, POKEMON_WIDTH_BIG, POKEMON_HEIGHT_BIG);
			}
			else {
				ctx.drawImage(image.obj, Math.floor(((Renderer.numRTicks + randInt) % 20)/5) * POKEMON_WIDTH, POKEMON_HEIGHT * followDirection, POKEMON_WIDTH, POKEMON_HEIGHT, renderPosX + offsetX, renderPosY + offsetY + (canIdleJump && (Renderer.numRTicks + randInt) % 10 < 5 ? -2 : 0), POKEMON_WIDTH, POKEMON_HEIGHT);
			}
			
			//ctx.drawImage(image.obj, POKEMON_WIDTH * followDirection, Math.floor(((Renderer.numRTicks + randInt) % 10)/5) * POKEMON_HEIGHT, POKEMON_WIDTH, POKEMON_HEIGHT, renderPosX + offsetX, renderPosY + offsetY + (canIdleJump && (Renderer.numRTicks + randInt) % 10 < 5 ? -2 : 0), POKEMON_WIDTH, POKEMON_HEIGHT);
